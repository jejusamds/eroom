...
$no = count($list);
foreach ($list as $row) {
    $app_type = $row['f_applicant_type'] === 'P' ? '개인' : '단체';
    echo "<tr>";
    echo "<td>{$no}</td>";
    //echo "<td>" . $row['f_applicant_type'] === 'P' ? '개인' : '단체' . "</td>";
    echo "<td>" . safeAdminOutput($app_type) . "</td>";
    echo "<td>" . safeAdminOutput($row['f_title']) . "</td>";
    echo "<td>" . safeAdminOutput($row['f_part_title']) . "</td>";
    echo "<td>" . safeAdminOutput($row['f_field_title']) . "</td>";
    echo "<td>" . safeAdminOutput($row['f_event_title']) . "</td>";
    echo "<td>" . safeAdminOutput($row['f_user_name']) . "</td>";
    echo "<td>" . safeAdminOutput($row['f_tel']) . "</td>";
    echo "<td>" . safeAdminOutput($row['f_email']) . "</td>";
    echo "<td>" . safeAdminOutput($row['f_birth_date']) . "</td>";
