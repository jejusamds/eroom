...
if ($total > 0) {
    ...
    $sql = "SELECT o.*, m.f_user_name, m.f_org_name, f_member_type FROM {$this_table} o LEFT JOIN df_site_member m ON o.f_user_id = m.f_user_id ORDER BY o.idx DESC LIMIT {$offset}, {$page_set}";
    $list = $db->query($sql);
}
?>
<div class="pageWrap">
                    ...
                    <?php if ($total > 0): ?>
                        <?php foreach ($list as $i => $row): ?>
                            <tr>
                                ...
                                <td><?= $row['f_member_type'] === 'P' ? '개인' : '단체' ?></td>
                                <td><?= $row['f_user_name'] ? $row['f_user_name'] : $row['f_org_name'] ?></td>
