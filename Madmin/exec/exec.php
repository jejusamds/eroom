...
switch ($mode) {
    ...
    case 'delimg_competition':
        ...
        // DB에서 파일명만 조회
        $row = $db->row("SELECT f_image FROM df_site_{$table} WHERE idx = {$idx}");
        if ($row && !empty($row['f_image'])) {
            // 실제 파일 삭제: /userfiles/{table}/{filename}
            @unlink($_SERVER['DOCUMENT_ROOT'] . "/userfiles/{$table}/" . $row['f_image']);
            //$sql = " Delete From df_site_{$table} Where idx = '" . $idx . "' ";
            $sql = "UPDATE df_site_{$table} SET f_image = '' WHERE idx = '" . $idx . "' ";
            $db->query($sql);
            echo "Y";
        } else {
            echo "N_db";
