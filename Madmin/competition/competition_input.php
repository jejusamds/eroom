...
<script language="JavaScript">
    function inputCheck(f) {
        ...
        // 참가부문 최소 1개 입력 체크
        var partInputs = document.getElementsByName('parts[]');
        var partFilled = 0;
        for (var i = 0; i < partInputs.length; i++) {
            if (partInputs[i].value.trim() !== '') partFilled++;
        }
        if (partFilled < 1) {
            alert('참가부문을 최소 1개 입력하세요.');
            if (partInputs.length > 0) partInputs[0].focus();
            return false;
        }

        // 종목분야 최소 1개 입력 체크
        var fieldInputs = document.getElementsByName('fields[]');
        var fieldFilled = 0;
        for (var i = 0; i < fieldInputs.length; i++) {
            if (fieldInputs[i].value.trim() !== '') fieldFilled++;
        }
        if (fieldFilled < 1) {
            alert('종목분야를 최소 1개 입력하세요.');
            if (fieldInputs.length > 0) fieldInputs[0].focus();
            return false;
        }

        // 참가종목 최소 1개 입력 체크
        var eventInputs = document.getElementsByName('events[]');
        var eventFilled = 0;
        for (var i = 0; i < eventInputs.length; i++) {
            if (eventInputs[i].value.trim() !== '') eventFilled++;
        }
        if (eventFilled < 1) {
            alert('참가종목을 최소 1개 입력하세요.');
            if (eventInputs.length > 0) eventInputs[0].focus();
            return false;
        }
    ...
    function deleteImage(idx, field) {
        ...
        var fd = new FormData();
        fd.append('mode', 'delimg_competition');
        fd.append('table', '<?= $table ?>');
        fd.append('idx', idx); fd.append('field', field);
        xhr.onload = function () {
            if (xhr.responseText.trim() === 'Y') {
                document.getElementById(field + '_prev_img').remove();
                document.getElementById(field + '_del_btn').remove();
            } else {
                console.error(xhr);
